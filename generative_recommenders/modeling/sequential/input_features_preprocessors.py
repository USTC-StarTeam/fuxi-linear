...
class LearnablePositionalEmbeddingInputFeaturesPreprocessor(
    ...
    def forward(
        ...
        if position_offsets is not None :
            positions = (torch.zeros(N, device=past_ids.device)[None, :] + position_offsets[:, None]).to(torch.long)
        else :
            positions = torch.arange(N, device=past_ids.device).unsqueeze(0).repeat(B, 1)
        
        user_embeddings = past_embeddings * (self._embedding_dim**0.5) + self._pos_emb(positions)
        user_embeddings = self._emb_dropout(user_embeddings)
