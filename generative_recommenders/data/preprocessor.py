...
class KuairandDataProcessor(DataProcessor) :
    def __init__(self, prefix, expected_num_unique_items, expected_max_item_id):
        super().__init__(prefix, expected_num_unique_items, expected_max_item_id)
    
    def output_format_csv(self) -> str:
        return f"tmp/processed/{self._prefix}/sasrec_format.csv"
...
def get_common_preprocessors() -> Dict[
    ...
    kuairand_pure_dp = KuairandDataProcessor(
        prefix='kuairand-pure',
        expected_max_item_id = 7582 + 1,
        expected_num_unique_items = 7582 + 1, 
    )
    kuairand_1k_dp = KuairandDataProcessor(
        prefix='kuairand-1k',
        expected_max_item_id = 18054 + 1,
        expected_num_unique_items = 18054 + 1, 
    )
    kuairand_27k_dp = KuairandDataProcessor(
        prefix='kuairand-27k',
        expected_max_item_id = 131090 + 1,
        expected_num_unique_items = 131090 + 1,
    )
    kuairec_dp = KuairandDataProcessor(
        prefix='kuairec',
        expected_max_item_id = 9958 + 1,
        expected_num_unique_items = 9958 + 1,
    )
    return {
        "ml-1m": ml_1m_dp,
        "ml-20m": ml_20m_dp,
