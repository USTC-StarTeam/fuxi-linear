...
def train_fn(
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        average_train_elapse = train_elapse / (epoch + 1)
        average_eval_elapse = eval_elapse / (epoch + 1)
        logging.info(f'train speed: {average_train_elapse: .4f}s/epoch, eval speed: {average_eval_elapse: .4f}s/epoch')
        last_training_time = time.time()

    average_eval_elapse = eval_elapse / num_epochs
    average_train_elapse = train_elapse / num_epochs
    logging.info(f'average eval time: {average_eval_elapse: .4f}s/epoch')
    logging.info(f'average train time: {average_train_elapse: .4f}s/epoch')
    
    logging.info(f'model desc: {model_desc}')
